...
class AbbrSchoolMatcher:
    ...
    def abbreviation_search(self,
        ...
        """
        for school in zip(schools_df["Name"], schools_df["Address"]):
            for match in re.finditer(pattern, school[0].strip().lower()):
                ...
                selected_school.append((school[0].title(), school[1].title(), round(score, 4)))

        selected_school.sort(key=lambda x: x[2], reverse=True)
